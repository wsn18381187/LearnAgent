...
def execute_sub_task(sub_task_description:str, flow_messages:list) -> str:
    """
    Execute a single sub-task using flow_entrance (unified entry point).
    
    Sub-tasks are always COMPLEX (not EXTREME) — they don't need condition_flow
    recursion. flow_entrance handles:
    - Ordinary tool calls (search, read_file, etc.) → JSON function call
    - write_file calls → CodeAct mode (avoids JSON escaping issues)
    
    Pure analysis sub-tasks can respond directly without tool calls.
    """
    try:
        result = flow_entrance(
            difficulty="COMPLEX",
            messages=[
                {"role": "system", "content": SUB_TASK_SYSTEM_PROMPT},
                {"role": "user", "content": SUB_TASK_USER_PROMPT.format(
