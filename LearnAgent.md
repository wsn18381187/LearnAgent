# LearnAgent 
*开发中的agent框架学习项目*

## V0.1版本说明
本项目为个人学习、开发的Agent项目，欢迎讨论。当前版本基于OpenAI SDK和ChromaDB进行开发。
LearnAgent是一个agent框架，接入OpenAI兼容的模型即可使用。LearnAgent具有调用工具、自动强弱模型选择、调用log记录等基础agent功能外，还有通过RAG和用户画像机制实现的双重记忆功能，可以通过刻画用户画像和出发历史记录搜索来提供更加个性化、一体化的回应。另外针对agent使用场景中的复杂任务，我设计了一个*Condition Flow*模式，专门用来通过提取问题、分析问题、制定规划、分步处理的循环来将复杂问题逐步拆解并尝试解决。

*目前在更新用户画像这一步，我尝试了很多prompt，但是LLM还是会控制不住超出prompt中设定的用户事实、最近聊天总结等内容的上限。目前通过程序截断的方式来强行确保实现目标*
*搜索功能使用的是Tavily的API，想使用的话可以去官网注册一个账号，免费版一个月有1000次使用*

### 快速启动
**创建环境**
```bash
conda create -n la python==3.10
conda activate la
pip install openai python-dotenv chromadb
```
然后cd至LearnAgent/code-v0_1所在的文件路径

**配置API**
+ 在当前工作目录下创建一个.env文件用来管理你的所有API KEY，包括调用模型的API KEY和搜索API KEY
+ 在`functions/auto_configuration.py`中设定好Base URL，模型名称和其它参数，可以设置强模型和弱模型搭配使用
+ 在`functions/get_embedding.py`中，选择好你想使用的embedding模型，并且确定之后不要更改，这是实现RAG向量库的embedding核心，不想改的话用我的同款模型也可以，不过需要一个OpenRouter API-KEY。其实这种小模型本地部署也行。

**用户画像**

直接复制下面的内容，创建一个`user_info`文件夹，在此文件夹下创建一个`user_image.json`并粘贴即可
```json
{
  "basic_info": {},
  "preferences": [],
  "facts": [],
  "chat_history": [],
  "last_interaction": ""
}
```

**启动程序**
```python
python main.py
```
输入clear更新对话session
输入exit退出对话

## 开发前的构想
### 功能设计

基于Python和TypeScript语言进行开发，使用LangChain框架构建的Agent。可以通过历史对话信息构建用户画像，提供更加个性化的回答。同时内部具有多轮分析和决策过程，通过多层推理实现精确判断与回答

### 交互方式

初始阶段先搭建核心内容，在命令行中完成单次交互；

后续规划通过html页面搭建可交互UI，界面类似主流AI网页，具有切换对话、多轮对话、对话管理、上传文件、模型选择和设置的功能

### 核心决策

首先需要对当前任务进行判断（是否可以直接回答）

这一个步骤中间需要加入“获取历史相关对话信息”的步骤，即从前几轮对话和其他历史对话中搜索是否存在相关信息，如果有将其也作为输入信息之一进行判断与分析

如果认为当前任务比较复杂，则进入规划、工具调用、分析、决策步骤

+ 规划：分析任务内容并设计出几个不同的任务阶段
+ 工具调用：进入某个阶段后，将可用的工具的信息告诉模型并提供使用方法
+ 分析：根据当前任务阶段的要求和刚刚获取的工具的信息来分析，是否可以达成当前阶段任务目的，不论是否可以都输出当前分析的成果
+ 决策：根据某阶段的分析成果来判断是否进入下一阶段，并将当前阶段的分析成果作为历史信息告知下一阶段

### 历史记忆

历史记忆由3个部分组成：

+ 当前对话中的历史对话，需要通过长短记忆/RAG的方式总结历史对话中与当前任务相关的有效信息并提取出来、作为参考信息给当前模型而不是直接将对话历史输入给模型
+ 之前的对话中可能有用的内容，可以将之前的对话历史总结成简短的markdown，或者更高级一点通过BERT分析相关性来选取可能有用的内容
+ 用户画像，这个内容需要在用户的使用过程中反复总结得到用户的相关信息，作为参考内容

### 其他工具

+ 文件读、写工具，用于基本的历史对话总结、读取，以及后续的用户输入文件功能，或者后续的代码功能
+ 通用搜索API，可能使用Google Search，可以先用fake API替代一下
+ 特定搜索API，具体看需求和可用的API，例如论文搜索可以使用arxiv的API，帖子搜索可以用reddit/rednote的API




## 开发日志
*前几天的忘记写了，先总结一下*

已经完成基本的agent多轮对话框架，具有多模型自动切换、多种工具调用（联网搜索、执行基本终端指令、时间获取、询问用户）、多轮对话自动保存、用户画像提取与自动更新、模型调用log记录功能

**4.27**
+ 拓展了用户画像功能，不仅记录用户的基本信息、行为特点和偏好等，还记录最近的对话内容，用来给模型提供最近对话内容的参考，构成历史对话内容的一级记忆
+ 使用基于chromaDB开发的RAG向量库构成第二级记忆系统。chromaDB会根据特定id、向量、文本构成的基本数据单元进行管理，并提供了很方便的初始化、存储管理、向量搜索功能。其中将文本转换为向量我使用了OpenAI SDK格式调用的embedding模型。然后我封装了两个函数，一个是给LLM的tool，调用的时候会根据询问的内容进行embedding然后去数据库中找聊天记录，返回的聊天记录还会被小模型处理一次进行消息提炼；另一个是在每次对话结束之后，当对话历史文件保存好了，以滑动窗口的形式对短片的聊天记录进行embedding然后更新到数据库中。
+ 通过这个二级记忆系统，LearnAgent实现了既能快速响应最近的事件，又能在需要更多信息的时候动态地从数据库中找相关信息

**4.28**
+ 由于梁圣牛逼，我决定将基座模型切换为deepseek-v4-flash和deepseek-v4-pro。但由于deepseek会把reasoning的内容单独放到返回体中的某一个位置，为了兼容性起见，我先给请求模型的函数添加一个extra_body用来开启deepseek的非思考模式，以后再扩展一下怎么记录这个reasoning。此外deepseek官网的API返回的template中不包含cost。虽然OpenAI SDK已经给出了很通用的接口，但不同的模型返回的格式仍然存在不小差异，尤其是各家的器件模型。后续开发可能考虑细化get_model_response，根据不同的平台/模型自动配置多轮对话/log记录的机制
+ 添加了文件读取工具。其实之前的终端功能就能进行简单的文件读取了，但是对于更多格式的文件难以实现读写，所以有必要开发专门的文件读取工具。目前仅开发了常见文件的读取功能，后续会拓展对于超长文件的自动识别、RAG压缩功能使其更加通用。哦对了，这个tool目前是由LearnAgent自行完成的。
+ 构思了一下agent的核心状态机，当Router判断当前任务非常复杂需要进行规划然后分步骤进行时，就会进入状态机工作流，进入“任务分析-分布规划-执行子任务-汇总结果并验收”的循环，每一轮循环后会单独判断结果，满足需求后退出循环

**4.30**
+ 设计了LearnAgent的核心功能：*Condition Flow*。这个功能是用来处理复杂任务的一套信息流模式。与直接输出处理方案不同，进入Condition Flow模式之后，首先会分析当前的任务背景、条件、目标，然后将整个任务拆分为几个子任务，每个子任务有各自的条件和输入输出要求。顺序执行完子任务之后，会对当前的执行结果进行判断，如果符合预期结果则进行总结并输出，如果不符合则进一步进行任务分析、划分。这种处理方式相对于直接进行多步思考的区别是，Condition Flow过程中只会传递每一步的执行结果，而不会传递每一步的完整执行过程。由于任务经过划分进行多步处理，这样的处理方式可以在保留关键目标信息的同时使上下文不太臃肿，适合复杂任务的处理场景

**5.1**
+ 修复了大量模型返回时触发异常的bug，增加了很多容错机制，确保报错有迹可循的情况下整体不会因为某一次调用失败直接卡住。
+ 发现agent在执行复杂文件写入的功能时，如果直接通过tool调用的话，处于输出格式的要求，会多套一层字符转义，导致LLM很容易输出出错。因此参考主流agent的方法，使用CodeAct模式，让LLM直接搭建写入文件的python脚本，然后将要写入的内容放在python代码中的三引号内，这样就不需要转义，只需要规定好LLM可以使用的python函数后执行该python即可实现写入功能

**5.6**
+ 给Condition Flow的入口添加了更多工具，之前我想的是直接进入flow然后开始规划、执行，但是今天在测试时发现有时候给出的任务需要额外的信息补充后，才能进行任务提取以及后续的执行。所以我修改了一下flow entrance，提供了更多工具而不是只有入口
+ 修改了一下condition flow内部调用工具的逻辑，不是先进入codeact再套一层python壳执行指令，而是直接可用全部工具，在需要写入的时候才使用codeact
+ 添加了进程管理机制。我发现之前的终端工具没有进程管理回收机制，导致在执行长任务多次调用终端的时候，在整个程序结束前终端不会自动关闭，可能会积累很多终端在内存中，这在工程上是不被允许的。因此我修改了终端工具。将调用终端的任务分为两种，一种只是查看信息的短期窗口，这个在运行完之后或运行超时时直接关掉；另一种是需要在后台长时间运行的窗口，这一种不会直接关掉而是保留在后台，并在创建的时候记录进程号、指定日志输出目录，这样agent就能查看运行进度并决定是否关闭。为了区分这两种不同的用途，我给终端工具添加了一个`background`参数，由模型在使用工具时决定使用什么类型的终端

**6.12**
+ 给终端工具加了个用户确认机制，用户会先看到终端中要执行的指令并判断是否执行
+ 加了个简单的终端菜单，目前还没有添加新功能，构思是添加一下配置各种参数的设置功能以及不同的执行模式（类似自动模式、接受编辑模型等）

**6.13**
+ 最近调试的时候发现只是修改设置不进行聊天（messages中仅包含系统提示词时）不能进行embedding，加了一条检查messages长度的机制
+ 添加config查看当前设置功能以及修改当前配置功能。将之前存放在环境变量中的API KEY和硬编码的模型配置放到了config.json中，并设计了检查配置是否完整的功能
+ 发现CodeAct模式适合写长文件，但是不适合进行局部修改，于是我添加了一个edit_file工具，类似git diff的做法，仅让模型给出需要修改的部分地方并进行匹配修改
+ 小修了一下用户画像机制，强制要求模型限制每一条描述的长度，减少用户画像的总长度
+ 修复了一下edit_file功能的DESCRIPTION的bug
+ 更新了router的功能，不再判断是否使用工具

**6.17**
+ 优化了终端输入方式，使用现在Cli中主流的输入库`prompt_toolkit`中的输入模块进行输入而不是原来单一的`input()`，支持了光标移动、编辑，同时支持跨对话的上下键输入历史
+ 向编辑文件、写入文件、读取文件的操作添加了同意机制，用户可以逐个确认是否进行这些操作。后续更新中会添加`自动模式`、`可读模式`等用于调节Agent的自主程度

**6.18**
+ 使用`prompt_toolkit`中的选择模块优化了文件操作和终端指令中的选择功能
+ 修复了终端指令中控制终端长期运行/关闭的逻辑bug，使用全平台通用的指令
+ 增加了自动模式配置功能，分别有`自动读`、`自动编辑`、`自动执行`三个配置，可以针对不同的工具来选择逐个确认/全自动执行

**6.28**
+ 添加了主程序中输入检查功能，当为空时不作为消息输入，避免在agent运行期间不小心按到空格导致后续对话自动接收空输入并输出
+ 添加了一个`/btw`功能，用来在不打断当前主要对话的情况下问一个相关问题
+ 更新了`ask_user_more_info`功能，之前只是个测试tool，现在改成了可以让agent向用户同时问多个问题、并有预设选项的tool。当预设选项中没有用户想要选择的选项时，还可以自行输入对应的回答